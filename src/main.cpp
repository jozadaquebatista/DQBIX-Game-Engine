...
void init()
{
	...
	anim->getTransform()->setScale(0.2f, 0.2f, 0.2f);

	spr = new Sprite("data/monkey_n.png");
	spr->setName("spr");
	spr->getTransform()->setTranslation(250.0f, 250.0f, 0.0f);
	spr->getTransform()->rotate_transform(0.0f, 0.0f, 45.0f);
	spr->attachScript(teste);

	spd = new Sprite("data/monkey_d.png");
	spd->setName("spd");
	spd->getTransform()->setTranslation(128.0f, 64.0f, 0.0f);
	spr->addChild(spd);

	tex = new Text("andy.ttf", 32);
	tex->setText("Hello World!");
	tex->setColor(1.0f, 1.0f, 1.0f, 1.0f);
	tex->getTransform()->setTranslation(120, 120, 0);
	tex->getTransform()->setScale(1.0f, -1.0f, 1.0f);
